...
def pcli_create_vms_from_iso(env_json_py, mgt, vi):
    ...
    if vi:
        print("Building nested vi cluster (estimated time = 40min):")
        i=0
        while i < len(env_json_py["nested_esxi_servers"]["vi_host_specs"]):
            print("    Creating ESXi Host"+ str(i)+": "+env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["name_of_vm"])
            # Create temp PowerCLI script for populated input variables
            shutil.copy(create_vm_from_iso_script_name, str(i)+"_"+create_vm_from_iso_script_name)
            #new_create_vm_from_iso_script_name = str(i)+"_"+create_vm_from_iso_script_name
            temp_script.append(str(i)+"_"+create_vm_from_iso_script_name)
            # Find and Replace Server Input Variables from Script
            search_and_replace_in_file("ID:SIV-001", env_json_py["physical_server"][env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["deploy_to_physical_host"]]["ip_address"], temp_script[i])
            search_and_replace_in_file("ID:SIV-002", env_json_py["physical_server"][env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["deploy_to_physical_host"]]["username"], temp_script[i])
            search_and_replace_in_file("ID:SIV-003", env_json_py["physical_server"][env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["deploy_to_physical_host"]]["password"], temp_script[i])
            search_and_replace_in_file("ID:SIV-004", env_json_py["physical_server"][env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["deploy_to_physical_host"]]["deploy_vms_to_this_datastore"][env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["deploy_to_datastore"]], temp_script[i])
            search_and_replace_in_file("ID:SIV-005", env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["CD_path"], temp_script[i])
            # Find and Replace VM Input Variables from Script
            search_and_replace_in_file("ID:VIV-001", env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["name_of_vm"], temp_script[i])
            search_and_replace_in_file("ID:VIV-002", env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["numCPU"], temp_script[i])
            search_and_replace_in_file("ID:VIV-003", env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["memoryGB"], temp_script[i])
            search_and_replace_in_file("ID:VIV-004", env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["harddisks"][0]["storage_GB"], temp_script[i])
            search_and_replace_in_file("ID:VIV-005", env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["harddisks"][1]["storage_GB"], temp_script[i])
            search_and_replace_in_file("ID:VIV-006", env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["harddisks"][2]["storage_GB"], temp_script[i])
            search_and_replace_in_file("ID:VIV-007", env_json_py["physical_server"][env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["deploy_to_physical_host"]]["deploy_vms_to_this_network"], temp_script[i])
            # Find and Replace Nested ESXi Input Variables from Script
            search_and_replace_in_file("ID:EIV-001", env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["nested_esxi_ip_address"], temp_script[i])
            search_and_replace_in_file("ID:EIV-002", env_json_py["physical_network"][1]["subnet_mask"], temp_script[i])
            search_and_replace_in_file("ID:EIV-003", env_json_py["physical_network"][1]["default_gateway"], temp_script[i])
            search_and_replace_in_file("ID:EIV-004", env_json_py["dns"][0], temp_script[i])
            search_and_replace_in_file("ID:EIV-005", env_json_py["dns"][1], temp_script[i])
            search_and_replace_in_file("ID:EIV-006", env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["nested_esxi_hostname"], temp_script[i])
            # Build Nested ESXi VM
            pcli_execute(temp_script[i])
            # Cleanup
            delete_script_file(temp_script[i])
            print("    Finished building ESXi Host"+ str(i)+": "+env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["name_of_vm"])
            i=i+1
...
def pcli_prep_hosts_for_vcf(env_json_py, mgt, vi): 
    ...
    if vi:
        ...
        while i < len(env_json_py["nested_esxi_servers"]["vi_host_specs"]):
            # Create list of nested hosts
            hostlist = hostlist+"\""+env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["nested_esxi_ip_address"]+"\", "
            # Lookup physical network portgroup
            search_and_replace_in_file("ID:SIV-006", env_json_py["physical_server"][env_json_py["nested_esxi_servers"]["vi_host_specs"][i]["deploy_to_physical_host"]]["deploy_vms_to_this_network"], prep_esxi_hosts_for_vcf_script_name)
            i=i+1
        # Remove comma from the last host
        hostlist = hostlist[:-2]
        print("    Host List: "+hostlist)
